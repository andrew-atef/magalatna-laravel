...
>
    ...
    <!-- Header Hero & Stores Filter Carousel -->
    <section class="mb-8">
        <div class="mb-6 rounded-2xl border border-sky-100 bg-sky-50/70 p-6 sm:p-8">
            <h1 class="text-2xl font-black text-slate-900 sm:text-3xl">عروض ومجلات التخفيضات في مصر اليوم</h1>
        ...
        <!-- أزرار المتاجر السريعة (Store Filter Pills) -->
        <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
            <a
                href="{{ route('home') }}"
                class="inline-flex shrink-0 items-center gap-1.5 rounded-full px-4 py-2 text-xs font-bold transition {{ empty($selectedRetailerSlug) ? 'bg-sky-600 text-white shadow-sm' : 'border border-slate-200 bg-white text-slate-700 hover:bg-slate-50' }}"
            >
                جميع المتاجر
            </a>
            @foreach ($retailers as $ret)
                <a
                    ...
                    class="inline-flex shrink-0 items-center gap-2 rounded-full border px-3.5 py-1.5 text-xs font-bold transition border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
                >
                    @if ($ret->logo_path)
    ...
    <!-- المجلات السارية حالياً (Active Flyers Grid) -->
    <section class="mb-12">
        <div class="mb-4 flex items-center justify-between">
            <h2 class="text-lg font-black text-slate-900 sm:text-xl">أحدث مجلات العروض السارية</h2>
            ...
            @forelse ($flyers as $flyer)
                ...
                <article class="group flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:shadow-md">
                    ...
                    <div class="flex flex-1 flex-col p-4">
                        ...
                        <h3 class="mt-1 line-clamp-2 text-sm font-bold text-slate-900 group-hover:text-sky-600">
                        ...
                        <div class="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-[11px]">
                            @if ($from->isFuture())
                                <span class="font-bold text-amber-600">يبدأ قريباً: {{ $from->format('d/m/Y') }}</span>
                                <span class="rounded bg-amber-50 px-2 py-0.5 text-[10px] font-bold text-amber-700">قريباً</span>
                            @elseif ($until->isPast())
    ...
    <!-- جدول / بطاقات أقوى السلع المخفضة (Hot Items Spotlight) -->
    @if ($hotItems->isNotEmpty())
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 class="mb-4 text-lg font-black text-slate-900">أقوى تخفيضات السلع والمنتجات اليوم</h2>
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                @foreach ($hotItems as $item)
                    <div class="flex flex-col rounded-xl border border-slate-100 bg-slate-50/70 p-3">
                        <span class="text-[11px] font-semibold text-sky-700">{{ $item->flyer->retailer->name }}</span>
                        <div class="mt-1 font-bold text-slate-900 text-xs line-clamp-2">{{ $item->product_name }}</div>
                        <div class="mt-auto pt-2">
                            @if ($item->old_price)
                                <span class="text-[11px] text-slate-400 line-through">{{ number_format((float) $item->old_price, 2) }} ج.م</span>
                            @endif
                            <div class="text-sm font-black text-emerald-600">{{ number_format((float) $item->sale_price, 2) }} ج.م</div>
                        </div>
                    </div>
                @endforeach
            </div>
